...
export function MarkdownContent({ source, className = "" }: { source: string; className?: string }) {
  ...
  return <div className={`markdown-content min-w-0 ${className}`}>
    {blocks.map((block, index) => {
      ...
      if (block.type === "code") return <pre key={index} className="my-4 max-w-full overflow-x-auto whitespace-pre rounded-lg bg-slate-950 p-4 font-mono text-sm leading-6 text-slate-100"><code className={block.language ? `language-${block.language}` : undefined}>{block.text}</code></pre>;
