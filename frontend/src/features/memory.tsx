...
export default function Memory() {
  ...
  const exportMemory = () => { const blob = new Blob([JSON.stringify(data ?? { messages: [], context: { items: [] } }, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "memory-export.json"; anchor.click(); URL.revokeObjectURL(url); };
  ...
  return <div className="space-y-6">
    <Card className="border-primary/30 bg-primary/[0.03]"><CardHeader><div className="flex items-start gap-3"><BrainCircuit className="mt-1 size-5 text-primary" /><div><CardTitle>Memory prüfen</CardTitle><CardDescription>Jede Erinnerung bleibt an Scope, Nachricht und Run gebunden.</CardDescription></div></div></CardHeader><CardContent className="grid gap-3 md:grid-cols-4">{(Object.keys(scope) as (keyof typeof scope)[]).map((key) => <Input key={key} aria-label={key} placeholder={key.replace("_id", " ID")} value={scope[key]} onChange={(e) => setScopeValue(key, e.target.value)} />)}<div className="flex gap-2 md:col-span-4"><Input aria-label="Memory durchsuchen" placeholder="Nach Inhalt suchen …" value={query} onChange={(e) => setQuery(e.target.value)} /><Button onClick={() => { setCursor(undefined); void load(); }}><Search className="mr-2 size-4" />Suchen</Button><Button variant="outline" onClick={exportMemory} disabled={!data}><Download className="mr-2 size-4" />Exportieren</Button><Button variant="destructive" onClick={() => void remove()}><Trash2 className="mr-2 size-4" />Scope löschen</Button></div></CardContent></Card>
