...
const germanSource=source=>/[äöüß]/i.test(source)||/(?:\b(?:bitte|einstellungen|projekt|aufgabe|speichern|löschen|bearbeiten|keine|noch|wähle|spalte|status|prüfung|änderung|fehlgeschlagen|übernahme)\b)/i.test(source);
const trText=source=>shipyardTranslations[shipyardLanguage()]?.[source]||shipyardTranslations.en?.[source]||(shipyardLanguage()==='en'&&germanSource(source)?'Translation unavailable':source);
...
const isUserAuthored=node=>!!node.parentElement?.closest(userAuthoredSelector);
const applyLanguage=()=>{const lang=shipyardLanguage();document.documentElement.lang=lang;const dictionary=shipyardTranslations[lang]||{};const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);nodes.forEach(node=>{const original=translationOriginals.get(node)||node.nodeValue;translationOriginals.set(node,original);if(isUserAuthored(node))return;const value=original.trim();const replacement=dictionary[value]||(lang==='en'&&germanSource(value)?'Translation unavailable':undefined);if(replacement)node.nodeValue=original.replace(value,replacement);else node.nodeValue=original;});document.querySelectorAll('[title],[aria-label],[placeholder]').forEach(node=>['title','aria-label','placeholder'].forEach(attribute=>{const originalKey=`${attribute}`;const original=translationOriginals.get(node)?.[originalKey]||node.getAttribute(attribute);const values=translationOriginals.get(node)||{};values[originalKey]=original;translationOriginals.set(node,values);if(original&&!node.matches(userAuthoredSelector)){const replacement=dictionary[original]||(lang==='en'&&germanSource(original)?'Translation unavailable':undefined);node.setAttribute(attribute,replacement||original);}}));document.querySelectorAll('[data-i18n-key]').forEach(node=>{const key=node.dataset.i18nKey;const visible=dictionary[key]||'Translation unavailable';node.querySelector('span')?.replaceChildren(document.createTextNode(visible));node.title=visible;});};
