...
if(canvas){
 ...
 const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.classList.add('workflow-lines');svg.setAttribute('aria-hidden','true');svg.innerHTML='<defs></defs>';canvas.prepend(svg);
 ...
 const edgeStyles=[['#176f8a','solid'],['#a84713','dashed'],['#287354','dotted'],['#6c4aa5','solid']];
 const path=(a,b,lane=0)=>{const start={x:snap(a.x),y:snap(a.y)},end={x:snap(b.x),y:snap(b.y)},mid=snap((start.x+end.x)/2)+lane*18;return `M ${start.x} ${start.y} H ${mid-12} Q ${mid} ${start.y} ${mid} ${start.y+12} V ${end.y-12} Q ${mid} ${end.y} ${mid+12} ${end.y} H ${end.x}`};
