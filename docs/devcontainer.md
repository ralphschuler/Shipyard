...
| Host CLI install for the provider command | Read-only, at the same absolute path. Codex's `codex.js` symlink target and its package tree (including a sibling platform package the launcher resolves) are mounted. The process runs with this image's `node`. Other host CLIs use the same mount. `/usr/local/bin` and the image Go/Node toolchain stay in the image. |
