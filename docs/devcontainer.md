...
| Allowlisted host CLI login files from `CODEX_HOME` or `~/.codex`, `CLAUDE_CONFIG_DIR` or `~/.claude` (plus `~/.claude.json` when that file exists), and `GROK_HOME` or `~/.grok` | Read-only, under the container home. The host directory itself is not mounted. A missing directory or a directory with no allowlisted file is skipped; container start continues. |
